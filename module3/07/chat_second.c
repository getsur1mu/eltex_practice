...
int OpenQueues(mqd_t *sendQueue, mqd_t *receiveQueue)
{
    ...
    *sendQueue = mq_open(QUEUE_TO_FIRST, O_CREAT | O_WRONLY, 0666, &attr);
    ...
    *receiveQueue = mq_open(QUEUE_TO_SECOND, O_CREAT | O_RDONLY, 0666, &attr);
...
int main()
{
    ...
    printf("Второй участник чата. Для выхода введите shutdown.\n");
